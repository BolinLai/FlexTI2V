...
class TextToVideoSynthesis(Model):
    ...
    def __init__(self, model_dir, *args, **kwargs):
        ...
        if kwargs["retrained_path"] is not None:  # = None
    ...
    def forward_with_vid_resample(self, input, vid, image_pos, add_vid_cond, use_ddpm_inversion, use_ddim_inversion,
        ...
        vid_embedding = None
        if vid is not None:
            vid = vid.to(self.device)  # (B, 3, M, 256, 256)
            # encode video to embedding
            vid_embedding = []
            for frame_idx in range(vid.size(2)):
                img = vid[:, :, frame_idx]
                img_embedding = self.autoencoder.encode(img).mean  # (B, 4, 32, 32)
            ...
            vid_embedding = torch.stack(vid_embedding, dim=2)  # (B, 4, M, 32, 32)

        # synthesis
        with torch.no_grad():
            num_sample = bs  # =B
            max_frames = self.config.model.model_args.max_frames  # 16
            latent_h, latent_w = 32, 32
            # with amp.autocast(enabled=True):
            with amp.autocast("cuda", enabled=True):
                if len(image_pos) == 1:
                    x0 = self.diffusion.ddim_sample_loop_with_vid_resample(
                        noise=torch.randn(num_sample, 4, max_frames, latent_h, latent_w).to(self.device),  # (B, 4, 16, 32, 32)
                        model=self.sd_model,
                        model_kwargs=[{"y": y.to(self.device)}, {"y": zero_y.to(self.device)}],
                        cond_vid=vid_embedding,  # (B, 4, M, 32, 32)
                        image_pos=image_pos,
                        guide_scale=guide_scale,  # =9.0
                        ddim_timesteps=ddim_step,  # =20
                        eta=0.0,
                        add_vid_cond=add_vid_cond,  # True
                        use_ddpm_inversion=use_ddpm_inversion,  # True
                        use_ddim_inversion=use_ddim_inversion,  # False
                        resample_iter=resample_iter,  # =0/1
                ...
                elif len(image_pos) >= 2:
                    x0 = self.diffusion.ddim_sample_multiframe(
                        noise=torch.randn(num_sample, 4, max_frames, latent_h, latent_w).to(self.device),  # (B, 4, 16, 32, 32)
                        model=self.sd_model,
                        model_kwargs=[{"y": y.to(self.device)}, {"y": zero_y.to(self.device)}],
                        cond_vid=vid_embedding,  # (B, 4, M, 32, 32)
